...
## Combining multiple `catch` expressions

When converting multiple subsequent `catch` expressions into `try ... catch ... end`,
consider whether they can be combined into a single `try ... catch ... end` block.
...
Instead of mechanically converting each `catch` separately:
...
Consider whether a single `try ... catch ... end` might be clearer:
...
Be aware that this changes the error handling semantics: with separate
`try ... catch ... end` blocks, if `bar(X)` throws an exception, `baz(Y)` will
still be executed. With a single block, an exception in `bar(X)` will prevent
`baz(Y)` from running. Choose the approach that matches your intended behaviour.
