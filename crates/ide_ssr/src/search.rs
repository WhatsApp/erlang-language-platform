...
use crate::matching::PlaceholderCache;
...
impl MatchFinder<'_> {
    ...
    fn slow_scan_node(
        ...
        // Cache the code body per body_origin to avoid redundant Salsa
        // queries. Within a single function/form, all AST nodes share
        // the same body, so we only need to fetch it once. This
        // dramatically reduces RwLock contention on Salsa's internal
        // storage when many threads are matching simultaneously.
        let mut cached: Option<(BodyOrigin, Arc<Body>)> = None;
        ...
        self.scope.fold(
            ...
            (),
            &mut |_acc, ctx| {
                // Reuse cached body if the body_origin hasn't changed
                let body = match &cached {
                    Some((origin, body)) if *origin == ctx.body_origin => Arc::clone(body),
                    _ => {
                        let body = ctx
                            .body_origin
                            .get_body(self.sema)
                            .expect("Could not get code Body");
                        cached = Some((ctx.body_origin, Arc::clone(&body)));
                        body
                    }
                };
                let code_body = fold_body(self.strategy, &body);
